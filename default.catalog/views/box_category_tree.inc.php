<?php
  $draw_branch = function($category, &$category_path, &$draw_branch) {
    echo '<li class="category-'. $category['id'] . (!empty($category['opened']) ? ' opened' : '') . (!empty($category['active']) ? ' active' : '') .'">' . PHP_EOL
       . '  <a href="'. htmlspecialchars($category['link']) .'"> '. $category['name'] . (settings::get('category_tree_product_count') ? ' <small class="pull-right">('. reference::category($category['id'])->num_products .')</small>' : '') .'</a>' . PHP_EOL;
